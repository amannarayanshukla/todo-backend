...
var commentedClass = invokeFunction(
...
var classAnonymousCommentedOneLine = invokeFunction(
...
test("not callables", function (t) {
  ...
  forEach(
    ...
    function (nonFunction) {
      t.equal(
        isCallable(nonFunction),
        false,
        inspect(nonFunction) + " is not callable"
      );
    }
  );

  t.test(
    "non-function with function in its [[Prototype]] chain",
    function (st) {
...
test("Arrow functions", { skip: arrows.length === 0 }, function (t) {
  ...
  t.ok(
    isCallable(weirdlyCommentedArrowFn),
...
test(
  ...
  function (t) {
    ...
    t.notOk(
      isCallable(commentedClassOneLine),
      "one-line class constructors with comments in the signature are not callable"
    );
    t.notOk(
      isCallable(classAnonymous),
      "anonymous class constructors are not callable"
    );
    t.notOk(
      isCallable(classAnonymousCommentedOneLine),
      "anonymous one-line class constructors with comments in the signature are not callable"
    );
    t.end();
  }
);
