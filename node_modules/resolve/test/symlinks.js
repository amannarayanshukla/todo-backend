...
try {
  ...
  fs.symlinkSync(
    path.join(__dirname, "resolver", "symlinked", "_", "symlink_target") + "\\",
    symlinkDir,
    "junction"
  );
}
...
try {
  ...
  // if fails then it is probably on Windows and lets try to create a junction
  fs.symlinkSync(
    path.join(__dirname, "..", "..", "source", "node_modules", "mod-a") + "\\",
    symlinkModADir,
    "junction"
  );
}
...
test("symlink", function (t) {
  ...
  resolve(
    "foo",
    { basedir: symlinkDir, preserveSymlinks: false },
...
test("async symlink from node_modules to other dir when preserveSymlinks = false", function (t) {
  ...
  resolve(
    "package",
    { basedir: basedir, preserveSymlinks: false },
    function (err, result) {
      t.notOk(err, "no error");
      t.equal(
        result,
        path.resolve(__dirname, "resolver/symlinked/package/bar.js")
      );
    }
  );
});
      ...
      resolve(
        ...
        function (err, actualPath) {
          ...
          st.deepEqual(
            ...
            map(
              preserveSymlinks
                ? [destPkg, destPkg, destPkg]
                : [sourcePkg, sourcePkg, sourcePkg],
              path.normalize
            ),
