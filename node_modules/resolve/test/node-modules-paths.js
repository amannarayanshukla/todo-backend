...
var verifyDirs = function verifyDirs(t, start, dirs, moduleDirectories, paths) {
  ...
  t.equal(
    keys(parsedDirs).length >= start.split(path.sep).length,
    true,
    'there are >= dirs than "start" has'
  );
  var foundModuleDirNames = keys(foundModuleDirs);
  t.deepEqual(
    foundModuleDirNames,
    moduleDirs,
    "all desired module dirs were found"
  );
  t.equal(
    keys(uniqueDirs).length,
    dirs.length,
    "all dirs provided were unique"
  );
  ...
  t.equal(
    keys(counts).length,
    1,
    "all found module directories had the same count"
  );
};
...
test("node-modules-paths", function (t) {
  ...
  t.test("with paths=function option", function (t) {
    var paths = function paths(
      ...
      return getNodeModulesDirs().concat(
        path.join(absoluteStart, "not node modules", request)
      );
    };
    ...
    verifyDirs(t, start, dirs, null, [
      path.join(start, "not node modules", "pkg"),
    ]);
  ...
  t.test("combine paths correctly on Windows", function (t) {
    ...
    t.equal(
      dirs.indexOf(path.resolve(start)) > -1,
      true,
      "should contain start dir"
    );
  ...
  t.test(
    "combine paths correctly on non-Windows",
    ...
    function (t) {
      ...
      var moduleDirectories = [
        "node_modules",
        "/Users/username/git/myProject/src",
      ];
      var dirs = nodeModulesPaths(start, {
        paths: paths,
        moduleDirectory: moduleDirectories,
