...
test("parse()", function (t) {
  ...
  t.test("transforms arrays to objects", function (st) {
    ...
    st.deepEqual(
      qs.parse("a[]=b&a[t]=u&a[hasOwnProperty]=c", { allowPrototypes: false }),
      { a: { 0: "b", t: "u" } }
    );
    st.deepEqual(
      qs.parse("a[]=b&a[t]=u&a[hasOwnProperty]=c", { allowPrototypes: true }),
      { a: { 0: "b", t: "u", hasOwnProperty: "c" } }
    );
    st.deepEqual(
      qs.parse("a[]=b&a[hasOwnProperty]=c&a[x]=y", { allowPrototypes: false }),
      { a: { 0: "b", x: "y" } }
    );
    st.deepEqual(
      qs.parse("a[]=b&a[hasOwnProperty]=c&a[x]=y", { allowPrototypes: true }),
  ...
  t.test("transforms arrays to objects (dot notation)", function (st) {
    st.deepEqual(qs.parse("foo[0].baz=bar&fool.bad=baz", { allowDots: true }), {
      foo: [{ baz: "bar" }],
      fool: { bad: "baz" },
    });
    st.deepEqual(
      qs.parse("foo[0].baz=bar&fool.bad.boo=baz", { allowDots: true }),
      { foo: [{ baz: "bar" }], fool: { bad: { boo: "baz" } } }
    );
    st.deepEqual(
      qs.parse("foo[0][0].baz=bar&fool.bad=baz", { allowDots: true }),
      { foo: [[{ baz: "bar" }]], fool: { bad: "baz" } }
    );
    st.deepEqual(
      qs.parse("foo[0].baz[0]=15&foo[0].bar=2", { allowDots: true }),
      { foo: [{ baz: ["15"], bar: "2" }] }
    );
    st.deepEqual(
    ...
    st.deepEqual(qs.parse("foo.bad=baz&foo[0]=bar", { allowDots: true }), {
      foo: { bad: "baz", 0: "bar" },
    });
    st.deepEqual(qs.parse("foo.bad=baz&foo[]=bar", { allowDots: true }), {
      foo: { bad: "baz", 0: "bar" },
    });
    st.deepEqual(qs.parse("foo[]=bar&foo.bad=baz", { allowDots: true }), {
      foo: { 0: "bar", bad: "baz" },
    });
    st.deepEqual(
      qs.parse("foo.bad=baz&foo[]=bar&foo[]=foo", { allowDots: true }),
      { foo: { bad: "baz", 0: "bar", 1: "foo" } }
    );
    st.deepEqual(
      qs.parse("foo[0].a=a&foo[0].b=b&foo[1].a=aa&foo[1].b=bb", {
  ...
  t.test("supports malformed uri characters", function (st) {
    st.deepEqual(qs.parse("{%:%}", { strictNullHandling: true }), {
      "{%:%}": null,
    });
    st.deepEqual(qs.parse("{%:%}="), { "{%:%}": "" });
    st.deepEqual(qs.parse("foo=%:%}"), { foo: "%:%}" });
  ...
  t.test("compacts sparse arrays", function (st) {
    st.deepEqual(qs.parse("a[10]=1&a[2]=2", { arrayLimit: 20 }), {
      a: ["2", "1"],
    });
    st.deepEqual(qs.parse("a[1][b][2][c]=1", { arrayLimit: 20 }), {
      a: [{ b: [{ c: "1" }] }],
    });
    st.deepEqual(qs.parse("a[1][2][3][c]=1", { arrayLimit: 20 }), {
      a: [[[{ c: "1" }]]],
    });
    st.deepEqual(qs.parse("a[1][2][3][c][1]=1", { arrayLimit: 20 }), {
      a: [[[{ c: ["1"] }]]],
    });
    st.end();
  });
  ...
  t.test("parses semi-parsed strings", function (st) {
    st.deepEqual(qs.parse({ "a[b]": "c" }), { a: { b: "c" } });
    st.deepEqual(qs.parse({ "a[b]": "c", "a[d]": "e" }), {
      a: { b: "c", d: "e" },
    });
    st.end();
  });
  ...
  t.test("continues parsing when no parent is found", function (st) {
    ...
    st.deepEqual(qs.parse("[]&a=b", { strictNullHandling: true }), {
      0: null,
      a: "b",
    });
    st.deepEqual(qs.parse("[foo]=bar"), { foo: "bar" });
  ...
  t.test("parses a string with an alternative RegExp delimiter", function (st) {
    st.deepEqual(qs.parse("a=b; c=d", { delimiter: /[;,] */ }), {
      a: "b",
      c: "d",
    });
    st.end();
  ...
  t.test("allows setting the parameter limit to Infinity", function (st) {
    st.deepEqual(qs.parse("a=b&c=d", { parameterLimit: Infinity }), {
      a: "b",
      c: "d",
    });
    st.end();
  ...
  t.test("allows overriding array limit", function (st) {
    ...
    st.deepEqual(qs.parse("a[0]=b&a[1]=c", { arrayLimit: 0 }), {
      a: { 0: "b", 1: "c" },
    });
    st.end();
  });
  ...
  t.test("allows for query string prefix", function (st) {
    st.deepEqual(qs.parse("?foo=bar", { ignoreQueryPrefix: true }), {
      foo: "bar",
    });
    st.deepEqual(qs.parse("foo=bar", { ignoreQueryPrefix: true }), {
      foo: "bar",
    });
    st.deepEqual(qs.parse("?foo=bar", { ignoreQueryPrefix: false }), {
      "?foo": "bar",
    });
    st.end();
  });
  ...
  t.test("parses string with comma as array divider", function (st) {
    st.deepEqual(qs.parse("foo=bar,tee", { comma: true }), {
      foo: ["bar", "tee"],
    });
    st.deepEqual(qs.parse("foo[bar]=coffee,tee", { comma: true }), {
      foo: { bar: ["coffee", "tee"] },
    });
    st.deepEqual(qs.parse("foo=", { comma: true }), { foo: "" });
    st.deepEqual(qs.parse("foo", { comma: true }), { foo: "" });
    st.deepEqual(qs.parse("foo", { comma: true, strictNullHandling: true }), {
      foo: null,
    });
    st.end();
  });
  ...
  t.test("does not allow overwriting prototype properties", function (st) {
    st.deepEqual(
      qs.parse("a[hasOwnProperty]=b", { allowPrototypes: false }),
      {}
    );
  ...
  t.test("can allow overwriting prototype properties", function (st) {
    st.deepEqual(qs.parse("a[hasOwnProperty]=b", { allowPrototypes: true }), {
      a: { hasOwnProperty: "b" },
    });
    st.deepEqual(qs.parse("hasOwnProperty=b", { allowPrototypes: true }), {
      hasOwnProperty: "b",
    });
  ...
  t.test("can return null objects", { skip: !Object.create }, function (st) {
    ...
    st.deepEqual(
      qs.parse("a[b]=c&a[hasOwnProperty]=d", { plainObjects: true }),
      expected
    );
    ...
    expectedArray.a.c = "d";
    st.deepEqual(
      qs.parse("a[]=b&a[c]=d", { plainObjects: true }),
      expectedArray
    );
    st.end();
  });
  ...
  t.test(
    ...
    function (st) {
      st.deepEqual(
        qs.parse("foo=" + urlEncodedNumSmiley, { charset: "iso-8859-1" }),
        { foo: "&#9786;" }
      );
      st.end();
    }
  ...
  t.test("does not interpret %uXXXX syntax in iso-8859-1 mode", function (st) {
    st.deepEqual(qs.parse("%u263A=%u263A", { charset: "iso-8859-1" }), {
      "%u263A": "%u263A",
    });
    st.end();
  });
