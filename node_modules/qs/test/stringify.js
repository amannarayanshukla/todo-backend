...
test("stringify()", function (t) {
  ...
  t.test(
    "with query prefix, outputs blank string given an empty object",
    function (st) {
  ...
  t.test("stringifies nested falsy values", function (st) {
    ...
    st.equal(
      qs.stringify({ a: { b: { c: null } } }, { strictNullHandling: true }),
      "a%5Bb%5D%5Bc%5D"
    );
  ...
  t.test("stringifies a nested object with dots notation", function (st) {
    ...
    st.equal(
      qs.stringify({ a: { b: { c: { d: "e" } } } }, { allowDots: true }),
      "a.b.c.d=e"
    );
    st.end();
  });
  ...
  t.test(
    "stringifies an array with mixed objects and primitives",
    function (st) {
      st.equal(
        qs.stringify(
          { a: [{ b: 1 }, 2, 3] },
  ...
  t.test(
    "stringifies an object inside an array with dots notation",
    function (st) {
      st.equal(
        qs.stringify(
          { a: [{ b: "c" }] },
  ...
  t.test(
    "uses indices notation for arrays when no arrayFormat is specified",
    function (st) {
  ...
  t.test(
    "uses indices notation for arrays when no arrayFormat=indices",
    function (st) {
      st.equal(
        qs.stringify({ a: ["b", "c"] }, { arrayFormat: "indices" }),
        "a%5B0%5D=b&a%5B1%5D=c"
  ...
  t.test(
    "uses repeat notation for arrays when no arrayFormat=repeat",
    function (st) {
      st.equal(
  ...
  t.test(
    "uses brackets notation for arrays when no arrayFormat=brackets",
    function (st) {
      st.equal(
        qs.stringify({ a: ["b", "c"] }, { arrayFormat: "brackets" }),
        "a%5B%5D=b&a%5B%5D=c"
  ...
  t.test("stringifies an empty value", function (st) {
    ...
    st.equal(
      qs.stringify({ a: { b: null } }, { strictNullHandling: false }),
      "a%5Bb%5D="
    );
  ...
  t.test(
    "stringifies an object with a null object as a child",
    { skip: !Object.create },
    function (st) {
  ...
  t.test(
    "skips properties that are part of the object prototype",
    function (st) {
  ...
  t.test("stringifies buffer values", function (st) {
    ...
    st.equal(
      qs.stringify({ a: { b: SaferBuffer.from("test") } }),
      "a%5Bb%5D=test"
    );
    st.end();
  });
  ...
  t.test("supports custom representations when filter=function", function (st) {
    ...
    st.equal(
      qs.stringify(obj, { filter: filterFunc }),
  ...
  t.test("can sort the keys", function (st) {
    ...
    st.equal(
      qs.stringify({ a: "c", z: "y", b: "f" }, { sort: sort }),
      "a=c&b=f&z=y"
    );
    st.equal(
      qs.stringify({ a: "c", z: { j: "a", i: "b" }, b: "f" }, { sort: sort }),
  ...
  t.test("RFC 1738 spaces serialization", function (st) {
    st.equal(
      qs.stringify({ a: "b c" }, { format: qs.formats.RFC1738 }),
      "a=b+c"
    );
    st.equal(
      qs.stringify({ "a b": "c d" }, { format: qs.formats.RFC1738 }),
      "a+b=c+d"
    );
    st.end();
  });
  ...
  t.test("RFC 3986 spaces serialization", function (st) {
    st.equal(
      qs.stringify({ a: "b c" }, { format: qs.formats.RFC3986 }),
      "a=b%20c"
    );
    st.equal(
      qs.stringify({ "a b": "c d" }, { format: qs.formats.RFC3986 }),
      "a%20b=c%20d"
    );
    st.end();
  });
