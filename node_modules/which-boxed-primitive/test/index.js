...
var primitives = [true, false, 42, NaN, Infinity, "", "foo"].concat(
  hasSymbols ? [Symbol(), Symbol.iterator] : [],
  hasBigInts ? BigInt(42) : []
);

var objects = [/a/g, new Date(), function () {}, [], {}];
...
test("isBoxedPrimitive", function (t) {
  t.test("unboxed primitives", function (st) {
    forEach([null, undefined].concat(primitives), function (primitive) {
      st.equal(
        null,
        whichBoxedPrimitive(primitive),
        debug(primitive, "is a primitive, but not a boxed primitive")
      );
    });
    st.end();
  });
  ...
  t.test("boxed primitives", function (st) {
    forEach(primitives, function (primitive) {
      ...
      st.equal(
        whichBoxedPrimitive(boxed),
        expected,
        debug(boxed, "is a boxed primitive: " + expected)
      );
    });
    st.end();
  });
  ...
  t.test("non-primitive objects", function (st) {
    forEach(objects, function (object) {
      st.equal(
        undefined,
        whichBoxedPrimitive(object),
        debug(object, "is not a primitive, boxed or otherwise")
      );
    });
    st.end();
  });
