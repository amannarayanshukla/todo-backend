...
describe('mquery', function () {
    ...
    describe('with 1 argument', function () {
      ...
      describe('that is an object', function () {
        ...
        it('works', function () {
          var m = mquery()
            .where('comment')
            .elemMatch({ author: 'joe', votes: { $gte: 3 } });
          assert.deepEqual(
            { comment: { $elemMatch: { author: 'joe', votes: { $gte: 3 } } } },
            m._conditions,
          );
        });
      });
  ...
  describe('near', function () {
    ...
    describe('with 0 args', function () {
      it('is compatible with geometry()', function (done) {
        var q = mquery()
          .where('x')
          .near()
          .geometry({ type: 'Point', coordinates: [180, 11] });
        assert.deepEqual(
          { $near: { $geometry: { type: 'Point', coordinates: [180, 11] } } },
          q._conditions.x,
        );
        done();
      });
    ...
    it('creates $geometry args for GeoJSON', function () {
      var m = mquery()
        .where('loc')
        .near({ center: { type: 'Point', coordinates: [10, 10] } });
      assert.deepEqual(
        { $near: { $geometry: { type: 'Point', coordinates: [10, 10] } } },
        m._conditions.loc,
      );
    });
    ...
    it('accepts spherical conditions', function () {
      var m = mquery()
        .where('loc')
        .near({ center: [1, 2], spherical: true });
    ...
    describe('executes', function () {
      ...
      describe('when criteria is passed with a callback', function () {
        it('if distinct arg was declared', function (done) {
          mquery(col)
            .distinct('age')
            .distinct({ name: 'mquery distinct' }, function (err, doc) {
              assert.ifError(err);
              assert.ok(doc);
              done();
            });
    ...
    describe('executes', function () {
      ...
      describe('when doc + callback passed', function () {
        it('works', function (done) {
          var m = mquery(col)
            .where({ _id: id })
            .update({ name: 'changed' }, function (err, num) {
              assert.ifError(err);
              assert.ok(1, num);
              m.findOne(function (err, doc) {
                assert.ifError(err);
  ...
  describe('exec', function () {
    ...
    describe('update', function () {
      ...
      describe('updateMany', function () {
        it('works', function (done) {
          mquery(col)
            .updateMany({ name: 'exec' }, { name: 'test' })
            .exec(function (error) {
      ...
      describe('replaceOne', function () {
        it('works', function (done) {
          mquery(col)
            .replaceOne({ name: 'exec' }, { name: 'test' })
            .exec(function (error) {
              assert.ifError(error);
              mquery(col)
                .findOne({ name: 'test' })
